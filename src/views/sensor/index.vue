<template>
    <div class="sensor-info">
        ...
        <van-nav-bar left-text="返回" left-arrow class="custom-nav-bar" title="传感器信息" right-text="新增"

                     @click-right="showAddSensorModal">
            <template #left>
        ...
        >
            <van-swipe-cell v-for="(sensor, index) in currentSensors"
                            :key="index">
                <div class="custom-van-row" @click="editSensor(sensor)">
                    <van-row style="display: flex;justify-content: space-between">
                        <van-col span="24">
                            <van-cell title="名称:" :value="sensor.baseName"></van-cell>
                        </van-col>
                    </van-row>
                    <van-row style="display: flex;justify-content: space-between">
                        <van-col span="24">
                            <van-cell title="位置:" :value="sensor.basePosition"></van-cell>
                        </van-col>
                    </van-row>
                    <van-row style="display: flex;justify-content: space-between">
                        <van-col span="24">
                            <van-cell title="在线:"
                                      :value="sensor.sensorStatus==1?'在线':'离线'"></van-cell>
                        </van-col>
                    </van-row>
                </div>
        ...
        <van-popup v-model="showModal" position="bottom">
            <van-tabs v-model="activeTab">
                <van-tab title="基础信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.baseNumber" label="传感器编号"/>
                        <van-field v-model="editingSensor.baseName" label="传感器名称"/>
                        <van-field v-model="editingSensor.baseClassify" label="传感器分类"/>
                        <van-field v-model="editingSensor.baseState" label="传感器状态"/>
                        <van-field v-model="editingSensor.basePosition" label="安装位置"/>
                        <van-field v-model="editingSensor.baseIp" label="传感器IP"/>
                        <van-field v-model="editingSensor.baseInstallationTime" label="安装时间" type="date"/>
                        <van-field v-model="editingSensor.baseInstallationMethod" label="安装方式"/>
                        <van-field v-model="editingSensor.baseTrnf" label="供电方式"/>
                        <van-field v-model="editingSensor.baseCommunicationMode" label="通讯方式"/>
                        <van-field v-model="editingSensor.relatedDeviceCode!=null?'是':'否'"  label="是否管理设备"/>
                    </div>
                </van-tab>
                <van-tab title="厂家信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.firmManufacturers" label="制造厂商"/>
                        <van-field v-model="editingSensor.firmBrand" label="品牌"/>
                        <van-field v-model="editingSensor.firmSerialNumber" label="序列号"/>
                        <van-field v-model="editingSensor.firmSize" label="尺寸"/>
                        <van-field v-model="editingSensor.firmWeight" label="重量"/>
                        <van-field v-model="editingSensor.firmRatedPower" label="额定功率"/>
                        <van-field v-model="editingSensor.firmFactoryDate" label="出厂日期" type="date" />
                        <van-field v-model="editingSensor.firmProductModel" label="产品型号"/>
                        <van-field v-model="editingSensor.firmPhoneNumber" label="联系电话"/>
                    </div>
                </van-tab>
                <van-tab title="施工单位信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.constructionUnit" label="施工单位"/>
                        <van-field v-model="editingSensor.constructionPerson" label="负责人"/>
                        <van-field v-model="editingSensor.constructionPhoneNumber" label="电话"/>
                    </div>
                </van-tab>
                <van-tab title="管理信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.maintenanceDepartment" label="维护部门"/>
                        <van-field v-model="editingSensor.maintenancePerson" label="维护员"/>
                        <van-field v-model="editingSensor.maintenancePhoneNumber" label="电话"/>
                        <van-field v-model="editingSensor.maintenanceUseDepartment" label="使用部门"/>
                    </div>
                </van-tab>
                <van-tab title="使用环境信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.environWorkHumidity" label="防爆等级"/>
                        <van-field v-model="editingSensor.environProtectionLevel" label="防护等级"/>
                        <van-field v-model="editingSensor.environWorkTemperature" label="工作温度"/>
                        <van-field v-model="editingSensor.environRadiate" label="辐射"/>
                    </div>
                </van-tab>
                <van-tab title="预留字段信息">
                    <div class="tab-content">
                        <van-field v-model="editingSensor.reserved1" label="预留字段"/>
            ...
            <div class="modal-footer">
                ...
                <van-button type="primary" @click="saveSensor()">确认</van-button>
            </div>
        </van-popup>
    </div>
</template>
    ...
    export default {
        data() {
            return {
                ...
                showModal: false,
                activeTab: 0,
                editingSensor: {}, // 当前编辑的传感器信息
                isSwiping: [], // 记录每个传感器是否在滑动状态
        ...
        methods: {
            initTable() {
                getSensorList({pageNum: this.pageNum, pageSize: this.pageSize}).then(res => {
                    if (res) {
                        this.currentSensors = res.rows
            ...
            onSelectDevice(device) {
                ...
                getSensorList({deviceCode: device_[1].deviceCode}).then(res => {
                    if (res.code == 200) {
                        console.log(res)
                        this.currentSensors = res.rows
                    }
            ...
            onSearch() {
                // this.currentSensors = [...this.sensors]
                // 点击搜索时的逻辑，比如可以触发一段函数去刷新数据

                getSensorList({baseName: this.searchName}).then(res => {
                    if (res.code == 200) {
                        this.currentSensors = res.rows
                    }
                })
                ...
                try {
                    ...
                    resData_.then(resData => {
                        if (resData.rows.length > 0) {
                            this.currentSensors.push(...resData.rows);
                            this.currentSensors = _.uniqWith(this.currentSensors, _.isEqual)
            ...
            async getTableData() {
                ...
                try {
                    const response = await getSensorList({deviceCode:this.selectedDevice.deviceCode,pageNum: this.pageNum, pageSize: this.pageSize})
                    // 直接获取数据
                    const data = response;
            ...
            showAddSensorModal() {
                this.resetEditingSensor();
                this.showModal = true;
            },
            editSensor(sensor) {
                this.showModal = true;
                getSensorInfo(sensor.id).then(res => {
                    if (res) {
                        console.log(res)
                        this.editingSensor = res.data.sensorInfo
                    }
                })
            },
            closeModal() {
            ...
            saveSensor() {
                if (this.editingSensor.id) {
                    let sensorInfoAndExtendVo = {
                        sensorInfo: this.editingSensor,
                        sensorExtendList: []
                    }
                    // sensorInfoAndExtendVo.sensorInfo.deviceCode = "99"
                    updateSensor(sensorInfoAndExtendVo).then(res => {
                        if (res.code == 200) {
                    ...
                    // sensorInfoAndExtendVo.sensorInfo.deviceCode = "99"
                    addSensor(sensorInfoAndExtendVo).then(res => {
                        if (res.code == 200) {
            ...
            deleteSensor(e) {
                const index = this.currentSensors.filter(sensor => sensor.id === e);
                // if (index !== -1) this.currentSensors.splice(index, 1);
                // TODO: 调用后端接口删除传感器信息
                console.log(index)
                this.$dialog.confirm({
                    title: '删除提示',
                    message: '确认删除传感器' + index[0].baseName,
                }).then(() => {
                    deleteSensor(e).then(res => {
                        if (res.code == 200) {
